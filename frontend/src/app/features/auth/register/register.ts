        ...
        <div class="modal-backdrop" (click)="closeGoogleModal()">
          <div class="google-modal-card" (click)="$event.stopPropagation()">
            <div class="gm-header">
              ...
              <p>Create a KaisaPaisa account using your Google profile</p>
            </div>
            ...
            <div class="custom-google-form">
              ...
              >
                {{ googleLoading() ? 'Signing up...' : 'Create Account with Google' }}
              </button>
            </div>
...
export class RegisterComponent implements OnInit, AfterViewInit {
  ...
  initializeGoogleIdentity(clientId: string) {
    ...
    if (google?.accounts?.id) {
      try {
        ...
        const btnElement = document.getElementById('googleRegisterBtnContainer');
        if (btnElement) {
              ...
              try {
                ...
                if (userRes.ok) {
                  ...
                  this.authService.googleLogin({
                    ...
                    error: (err) => {
                      this.googleLoading.set(false);
                      this.errorMessage.set(err.error?.message || err.error?.error || 'Google signup failed');
                    }
                  });
                  return;
                }
  ...
  selectGoogleAccount(name: string, email: string) {
    ...
    .pipe(catchError(err => {
      this.googleLoading.set(false);
      this.errorMessage.set(err.error?.message || 'Google account registration failed.');
      return of(null);
    }))
    .subscribe(res => {
      this.googleLoading.set(false);
  ...
  handleGoogleCredentialResponse(response: any) {
    ...
    this.authService.googleLogin({ credential: response.credential })
      .pipe(catchError(err => {
        this.googleLoading.set(false);
        this.errorMessage.set(err.error?.message || 'Google signup failed.');
        return of(null);
      }))
      .subscribe(res => {
        this.googleLoading.set(false);
